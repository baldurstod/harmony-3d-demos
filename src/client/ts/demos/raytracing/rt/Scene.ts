...
export class RayTracingScene {
	...
	public async loadModels(): Promise<{ materials: Material1[], faces: Uint8ClampedArray, aabbs: Uint8ClampedArray }> {
		...
		const sceneModels = RayTracingScene.parseModel(
		...
		RayTracingScene.MODELS_COUNT = sceneModels.length;
		...
		{
			RayTracingScene.MAX_NUM_FACES_PER_MESH = sceneModels.reduce(
			...
			faces = new Uint8ClampedArray(
				numFloatsPerFace *
				Float32Array.BYTES_PER_ELEMENT *
				RayTracingScene.MAX_NUM_FACES_PER_MESH *// TODO: fix that: variable faces per mesh
			...
			*/
			...
			for (let i = 0; i < RayTracingScene.MODELS_COUNT; i++) {
				const modelFaces = sceneModels[i]!.faces;
				let idx = i * numFloatsPerFace * RayTracingScene.MAX_NUM_FACES_PER_MESH;
		...
		// Prepare AABBS buffer
		{
			...
			RayTracingScene.MAX_NUM_BVs_PER_MESH = sceneModels.reduce(
				(val: number, obj) => Math.max(val, obj.AABBs.length),
				0,
			);
			const numFloatsPerBV = 12;

			RayTracingScene.AABBS_COUNT = RayTracingScene.MAX_NUM_BVs_PER_MESH * RayTracingScene.MODELS_COUNT;

			aabbs = new Uint8ClampedArray(numFloatsPerBV * Float32Array.BYTES_PER_ELEMENT * RayTracingScene.AABBS_COUNT)
			...
			*/
			...
			for (let i = 0; i < RayTracingScene.MODELS_COUNT; i++) {
				...
				let idx = numFloatsPerBV * RayTracingScene.MAX_NUM_BVs_PER_MESH * i;
			...
			{
				RayTracingScene.MATERIALS_COUNT = sceneMaterials.length;
				const numFloatsPerMaterial = 8;
				/*
