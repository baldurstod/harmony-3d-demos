...
fn rayColor(primaryRay: Ray, rngState: ptr<function, u32>) -> vec3<f32> {
	...
	for (var bounce = 0u; bounce < samplingParams.numBounces; bounce += 1u) {
		var intersectionVar = Intersection();

		if intersection(ray, &intersectionVar) {
			let material = materials[intersectionVar.materialIdx];

			if material.id == 4u {
				let emissionTexture = material.desc1;
				let emissionColor = textureLookup(emissionTexture, intersectionVar.u, intersectionVar.v);
				color += throughput * emissionColor;
				break;
			}

			var scatter = scatterRay(ray, intersectionVar, material, rngState);
			ray = scatter.ray;
...
fn textureLookup(desc: TextureDescriptor, u: f32, v: f32) -> vec3<f32> {
	...
	let j = u32(u2 * f32(desc.width));
	let i = u32(v2 * f32(desc.height));
